...
def build_razor_model_from_cfg(
    ...
    if cfg.get('cfg_path', None) and not cfg.get('type', None):
        cfg_path = cfg.get('cfg_path', None)
        package, cfg_path = cfg_path.split('::')
        cfg = Config.fromfile(cfg_path)
        kwargs = copy.deepcopy(cfg).pop("model")
        import importlib
        models_module = importlib.import_module(f'{package}.registry')
        MMDET_MODELS = models_module.MODELS
        MODELS._module_dict.update(MMDET_MODELS._module_dict)
        MMDET_TASK_UTILS = models_module.TASK_UTILS
        TASK_UTILS._module_dict.update(MMDET_TASK_UTILS._module_dict)
