...
from mmrazor.models.architectures.necks.layer import ln2d
...
def Meta_Encoder(type="conv", window_size=4):
    ...
    elif type == "conv_mobilenet_detection":
        module = lambda student_channel: nn.Sequential(nn.GroupNorm(student_channel, student_channel),
                                                       nn.ReLU(inplace=True),
                                                       nn.Conv2d(student_channel, student_channel // 8, (3,3), (1, 1), (1,1), bias=False),
                                                       nn.GroupNorm( student_channel // 8, student_channel // 8),
                                                       nn.Conv2d(student_channel // 8, student_channel, (3,3), (1, 1),(1,1), bias=False))
    ...
    def forward(self, student_feature, teacher_feature, inference_sampling=4, **kwargs):
        ...
        if self.training:
            ...
            for i in indices:
                ...
                if _weight != 0:
                    loss += align_loss(student_feature - t_output_feature, _velocity).mean() * _weight
            #self.iter_number += 1
            return loss, x
