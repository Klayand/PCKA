...
parser.add_argument('--arch', default='wrn_16_2_sskd', type=str, help='student network architecture')
parser.add_argument('--tarch', default='wrn_40_2_sskd', type=str, help='teacher network architecture')
...
tnet.load_state_dict(checkpoint,strict=False)
tnet.eval()
...
def train(epoch, criterion_list, optimizer):
    ...
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        optimizer.zero_grad()
        with torch.no_grad():
            ss_t_out, t_logits = tnet(input)
        ss_s_out, logits = net(input)
        loss=criterion_div(ss_s_out, ss_t_out, logits, t_logits, target)
        loss.backward()
        optimizer.step()
        ...
        if args.flops_calculate:
            ...
            class Analysis(nn.Module):
                ...
                def forward(self,data,target):
                    with torch.no_grad():
                       ss_t_out, t_logits = self.tnet(data)
                    ss_s_out,logits = self.net(data)
                    return self.dis(ss_s_out,ss_t_out,logits,t_logits,target)
