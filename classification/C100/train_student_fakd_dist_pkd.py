...
parser.add_argument('--arch', default='mobilenetV2_fakd', type=str, help='student network architecture')
...
parser.add_argument('--manual_seed', type=int, default=1221)
...
parser.add_argument('--kd_weight', type=float, default=0, help='weight for KD distillation')
parser.add_argument('--flow-weight', type=float, default=1.0, help='weight for Flow Align distillation')
...
log_txt = 'result/' + 'tarch' + '_' + args.tarch + '_' + \
          'arch' + '_' + args.arch + '_' + \
          'dataset' + '_' + args.dataset + '_' + f'_2倍_{args.i}' + f'_diff_layer_{args.layer_weight}' + f'_encoder_type_{args.encoder_type}' \
...
log_dir = str(os.path.basename(__file__).split('.')[0]) + '_' + \
          ...
          'seed' + str(
    args.manual_seed) + f'_diff_layer_{args.layer_weight}' + f'_{args.dirac_ratio}' + f'_encoder_type_{args.encoder_type}'
...
if args.tarch == "wrn_40_2":
    ...
    teacher_features = [16, 32, 64]
    teacher_sizes = [32, 16, 8]
elif args.tarch == "vgg13_bn" or args.tarch == "vgg13":
...
def train(epoch, criterion_list, optimizer):
    ...
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        loss_div = loss_div + (criterion_cls(logits,target)+criterion_div(logits, t_logits, target)) * args.kd_weight + flow_loss * args.flow_weight
        loss = loss_div
        loss.backward()
        optimizer.step()
