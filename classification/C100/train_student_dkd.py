...
parser.add_argument('--last-encoder', default=None, type=str, help='The Last Encoder')
...
parser.add_argument('--ce_weight', type=float, default=1, help='weight for CE')
parser.add_argument('--kd_weight', type=float, default=1, help='weight for KD distillation')
...
log_txt = 'result/' + 'tarch' + '_' + args.tarch + '_' + \
          'arch' + '_' + args.arch + '_' + \
          'dataset' + '_' + args.dataset + '_' + f'id_{args.i}_.txt'
...
net = model(num_classes=num_classes, last_encoder=args.last_encoder).cuda()
print(net)
...
def train(epoch, criterion_list, optimizer):
    ...
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        loss_div = loss_div + criterion_div(logits, t_logits, target, epoch) * args.kd_weight + \
                   criterion_cls(logits, target) * args.ce_weight
        loss = loss_div
...
if __name__ == '__main__':
    ...
    criterion_div = losses.DKDLoss(beta=6,alpha=1)
