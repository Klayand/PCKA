...
def train(epoch, criterion_list, optimizer):
    ...
    for batch_idx, (input, target) in enumerate(trainloader):
        ...
        with torch.cuda.amp.autocast(enabled=True):
            sfeatures, logits = net(input, is_feat=True)
            logits = logits.float()
        with torch.no_grad():
            tfeatures, t_logits = tnet(input, is_feat=True)
        loss_div = torch.tensor(0.).cuda()
        pkd_loss = 0.
        global transform_1,transform_2
        transform_list = [transform_1,transform_2]
        ii = 0
        for s, t in zip(sfeatures[1:], tfeatures[1:]):
            pkd_loss += criterion_div(s.float(), transform_list[ii](t.float()))
            ii+=1
        loss_div = loss_div + pkd_loss + criterion_div2(logits, t_logits, target) * args.kd_weight
        loss = loss_div
...
def test(epoch, criterion_cls, net):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, target) in enumerate(testloader):
            ...
            sf4, logits = net(input, is_feat=True)
...
if __name__ == '__main__':
    ...
    criterion_div = nn.MSELoss(reduction="mean")
    criterion_div2 = losses.KDLoss(alpha=0.5, beta=0.5, temperature=4)
    if args.evaluate:
        ...
        transform_1 = nn.Sequential(
            nn.BatchNorm2d(32),
            nn.Conv2d(32,32,(1,1),(1,1),(0,0),bias=False)).cuda()
        transform_2 = nn.Sequential(
            nn.BatchNorm2d(64),
            nn.Conv2d(64,64,(1,1),(1,1),(0,0),bias=False)).cuda()
        trainable_list.append(transform_1)
        trainable_list.append(transform_2)
