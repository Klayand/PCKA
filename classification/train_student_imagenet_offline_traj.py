...
parser.add_argument('--flow-loss-type', type=str, default="dist", help='loss type of flow')
parser.add_argument('--encoder-type', type=str, default="transformer", help='encoder type of flow')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    optimizer = optim.SGD(add_weight_decay(trainable_list,weight_decay=args.weight_decay,lr=args.init_lr),nesterov=True,
    ...
    def train(epoch, criterion_list, optimizer, iter_number):
        ...
        for batch_idx, (input, target) in enumerate(trainloader):
            ...
            loss_div = loss_div + flow_loss #+ criterion_div(logits,t_logits) + criterion_cls(logits,target)
            
            # for s_feature,t_feature in zip(s_features[1:],t_features[1:]):
            #     loss_div = loss_div + PKDLoss()(s_feature.float(),t_feature).float()
            ...
            for sampling in range(8,0,-1):
                ...
                with torch.no_grad():
                    for batch_idx, (inputs, target) in enumerate(testloader):
                        ...
                        output = net(input, inference_sampling=total_sampling,traj_point=sampling)
                        if isinstance(output, tuple):
                            logits = output[0]
                        else:
                            logits = output
                        total_output.append(logits)
                ...
                total_target = torch.cat(total_target,0).clone().detach().cpu().numpy()
                total_output = torch.cat(total_output,0).clone().detach().cpu().numpy()
                save_path = os.path.join("result",f"imagenet_traj_point_{sampling}.npz")
                np.savez(save_path,target=total_target,output=total_output)
    ...
    if args.evaluate:
        conv_path = "/home/project/FAKD/classification/checkpoint_2/resnet18_conv_dist.pth.tar"
        swin_path = "/home/project/FAKD/classification/checkpoint_2/resnet18_swin_dist.tar"
        print('load pre-trained weights from: {}'.format(swin_path))
        checkpoint = torch.load(swin_path,
                                map_location='cuda:{}'.format(args.gpu))
